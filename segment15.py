...
def process_and_draw_layers(bscan, layers_config, medline):
    ...
    # Adjust the B-scan image based on medline
    rows, cols = bscan.shape
    medline = medline.astype(int)
    # Create a mask to focus on the region below the medline
    mask = np.zeros_like(bscan, dtype=np.uint8)
    for col in range(cols):
        mask[medline[col]:, col] = 1  # Focus on layers below medline

    # Apply the mask to the B-scan image
    bscan_masked = cv2.bitwise_and(bscan, bscan, mask=mask)

    # Continue processing with the masked image
    # Apply Fourier Transform to the masked image
    oct_fft = fftpack.fft2(bscan_masked)
