...
def process_and_draw_layers(bscan):
    ...
    def draw_layer_lines(image, contours, min_contour_length, layer_colors):
        ...
        for idx, contour in enumerate(contours):
            if len(contour) > min_contour_length:
                ...
                for i in range(len(approx) - 1):
                    ...
                    cv2.line(output_image, start_point, end_point, layer_colors[idx % len(layer_colors)], 2)
    ...
    filtered_image_with_lines = draw_layer_lines(bscan, contours, min_contour_length, layer_colors)
