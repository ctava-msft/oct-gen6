def pcaEvaluation[DataDescriptors, Layers, Additional, Features, less]:
...
for i in 1:size[dataAll{1}, 1]:
    ...
    for s in 1:Features.numSamplesPCA:
...
def calculatePCACoeffs[dataAll, classes, validClasses, samples, percent]:
    coeffs = cell[0,1];
    ...
    for i in 1:size[dataAll{1}, 1]:
        ...
        # Move strptool path to the .
        [coeffs{i}, not , eigenValues{i}] = princomp[data];
        
        # Replacement:
        # [coeff,not ,latent] = pca[data', 0];
        # coeffs{i} = coeff;
