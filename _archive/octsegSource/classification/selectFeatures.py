def selectFeatures[features, classes, additional, description, SelectionOptions]:
...
if not isfield[SelectionOptions, 'corrRejectFirst']:
    SelectionOptions.corrRejectFirst = 0.0;

if not isfield[SelectionOptions, 'corrRejectOther']:
...
elif strcmp[SelectionOptions.method, 'StrCmp']:
    FeatureSelection.type = 0;
    description = description[(size(additional, 2] + 2):);
    
    idxColl = zeros[size(SelectionOptions.compareStrings, 1], numel[description]);
    for i in 1:size[SelectionOptions.compareStrings, 1];:
        # If the row has only one cell entry.
        if numel[SelectionOptions.compareStrings{i, 2}] == 0 :
            for k in 1:numel[description]:
            ...
            for k in 1:numel[description] :
                if (numel[strfind(description{k}, SelectionOptions.compareStrings{i, 1}]) > 0) andand ...:
...
elif strcmp[SelectionOptions.method, 'bestNext']:
...
# anymore, start adding again. Stop when no add/remove is possible
elif strcmp[SelectionOptions.method, 'alternate']:
    disp['Alternating feature selection.T];
    FeatureSelection.type = 1;
    featureRanking = zeros[1, size(features, 2]);
    ...
    for runNum in 1:size[crossValidationIdx, 1]:
        ...
        while numel[featuresSet] < SelectionOptions.numFeat:
            ...
            while actRate >= bestRate andand numel[featuresSet] < SelectionOptions.numFeat:
                featRate = zeros[size(featuresSet]);
                #disp[['Shrink: ' num2str(bestRate] ', ' num2str[actRate]]);
                for i in 1:numel[featuresSet]:
                    if SelectionOptions.disp:
                        disp (['Feature '  num2str[i]]);
                    
                    trainingFeatures = features[crossValidationIdx{runNum, 1}, featuresSet([1:(i-1] (i+1):]));
                    varTraining = var[trainingFeatures];
                    validFeatures = varTraining > 0.000001;
                    trainingFeatures = trainingFeatures[:, validFeatures];
                ...
                if actRate >= bestRate:
                    ...
                    featuresSet = [featuresSet[[1:(idx(1]-1) (idx[1]+1):])];    
                
            
            
            if (bestRateBefore == bestRate) andand (isequal (featuresSet, featuresSetBefore)):
                break;
...
def classRun[trainingFeatures, classTraining, ClassifyOptions]:
...
if strcmp[ClassifyOptions.classifier, 'Bayes']:
    [classResult, not , posterior] = classify[trainingFeatures, trainingFeatures, classTraining, ClassifyOptions.BayesSpec, ClassifyOptions.BayesPrior]; 
    Results.classResult{1} = classResult;
    Results.discriminators{1} = posterior[:,]; # Posteriors only valid for 2 class problem.
    ...
    for i in 1:numel[ClassifyOptions.classes]:
