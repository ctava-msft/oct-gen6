def selectGroup[features, classes, additional, GroupOptions]:
...
if GroupOptions.classBased :
    for i in 1:numel[GroupOptions.valid]:
        validAdder = classes == GroupOptions.valid[i];
        validCount = sum[double(validAdder]);
        disp[['Group ' num2str(GroupOptions.valid(i]) ' has ' num2str[validCount] ' datasets.T]);
...
count = 1;
for i in 1:numel[valid]:
    if valid[i]:
        for k in 1:size[additional, 2]:
