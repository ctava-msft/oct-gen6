def crossValidate[features, FeatureSelection, CrossValidation, ClassifyOptions, alternativeFeatures]:
...
for runNum in 1:size[CrossValidation.idx, 1]:
    ...
    if strcmp[ClassifyOptions.classifier, 'Bayes']:
        [classResult, not , posterior] = classify[testFeatures, trainingFeatures, classTraining, ClassifyOptions.BayesSpec, ClassifyOptions.BayesPrior];
        Results.classResult{runNum} = classResult;
        Results.discriminators{runNum} = posterior[:,]; # Posteriors only valid for 2 class problem.
