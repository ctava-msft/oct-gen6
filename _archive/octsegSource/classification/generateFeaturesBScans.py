def generateFeaturesBScans[DataDescriptors, Layers, Additional, Features, Types]:
...
# 
# The parameters structs are:
# DATADESCRIPTORS: 
# The field entries contain the data the features are generated from:
# - featureDataLoaded: Has the thickness/classes/age data been loaded into
# the worspace already? 
# - FeatureData: Struct with the data, containing thickness profiles
# (dataAll), classes, age, and the featureCollection (cell array with
# features, adder (filename, age, patinetID) and class already filled in.
# LAYERS: One entry for each layer thickness profile, either 0 or 1 (if the:
# features should be computed for the respective layer). Entries are:
# retina, rnfl, ipl, opl, onl, rpe, bv
# ADDITIONAL: Should additional data be copied into the feature collection?
# Entries are: filename, age, patientID.
# FEATURES: Parameter for feature generation. Entries are (with example value): 
# - normalize = 1; Normalize features to zero mean, std 1.
# - samples = 32; Number of sample regions for the standard features.
# - onlyAuto = 0; If set to 1, the manual corrections are ignored.
# - class = 'Diagnosis'; Class meta data tag.
# - numSamplesPCA = 64; Sample for the PCA features (thickness profiles are
# averaged to this number of samples before the PCA EV computation)
# - numEV = 10; Number of PCA features. 
# - pcaAllClasses = [0 1 2 3]; Classes for the "overall" PCA computation.
# - pcaNormalClasses = 0; Classes for the healthy/normal PCA computation.
# - normFocus = 1; Normalize the focus, e.g. do not use only the ScaleZ
# information of the B-Scans, but also the ScaleX information, i.e. the
# features are not computed on thickness measurements, but on area
# information.
# - ageNormalize = 1; Perform a linear age normalization in section.
# - ageNormalizeSamples = 32; Number of sections for the age normalization.
# - ageNormalizeRefAge = 50; Reference age.
# - ageNormalClass = 0; Class to compute the age nromalization parameters
# from.
# TYPES: Which features should be generated? Entries are:
# - completeStd: Overall min/max/mean/median
# - meanSections: Min/max/mean/median in sections 
# - ratioRetina: Ratio of the layer thickness to the complete retina
# thickness, averaged in quadrants.
# - pcaAll: PCA features, EV computed on all data.
# - pcaNormal: PCA features, EV computed on healthy data.
#
# Written by Markus Mayer, Pattern Recognition Lab, University of
# Erlangen-Nuremberg, markus.mayer@informatik.uni-erlangen.de
# Finalized Dezember 2015
...
if[DataDescriptors.featureDataLoaded]
    ...
    for i in 1:numel[DataDescriptors.filenameList]:
        ...
        if numDescriptor == 0:
            disp[[DataDescriptors.pathname DataDescriptors.filenameList{i} ': File is no OCT file.T]];
            continue;
...
for i in 1:size[featureTable, 1]:
...
def calculatePCACoeffs[dataAll, classes, validClasses, samples]:
    coeffs = cell[0,1];
    eigenValues = cell[0,1];
    
    valid = false[size(classes]);
    for i in 1:numel[validClasses]:
        validAdder = classes == validClasses[i];
        valid = valid or validAdder;
    

    dataAll = dataAll[valid];
    
    for i in 1:size[dataAll{1}, 1]:
        data = zeros[numel(dataAll], samples);
        for k in 1:numel[dataAll]:
        ...
        [coeffs{i}, not , eigenValues{i}] = princomp[data];
        # Replacement:
...
def loadLayers[filename, Header, DataDescriptors, Layers, Features]:
...
def loadAdditional[not , filename, Header, Additional]:
    adder = cell[0,1];
    adderCount = 1;
...
def getAdditonalDescription[Additional]:
...
def loadClass[pathname, filename, Features]:
    class = readOctMeta[[pathname filename], Features.class];
    class = round[class];


def readData[tags, pathname, filename, evaluatorName]:
    tagMan = [];
    if numel[tags] == 2 andand iscell[tags]:
        tagAuto = tags{1};
        tagMan = tags{2};
    else:
        if iscell[tags]:
...
def calculateLayerFeatures[data, Types, Features, pcaCoeffAll, pcaCoeffNormal]:
    ...
    featureData = [];
    for i in 1:size[featureCell, 1]:
        for k in 1:size[featureCell, 2]:
            featureData = [featureData featureCell{i, k}];
        
    


def getFeatureDescription[Types, Layers, Features]:
    ...
    featureCount = 1;
    for z in 1:numel[layerDescription]:
        for k in 1:numel[featurePerLayer] :
