def buildCrossValidation[features, classes, additional, options]:
...
# Reset random number generator so that we get the same results in every run.
rng['default']; 

numClasses = numel[options.classes];
CrossValidation.classesOriginal = classes;
...
for i in 1:numClasses:
    idxClasses{i} = [];
    for k in 1:numel[options.classes{i}]:
...
if options.equalSizeClasses:
    ...
    for i in 1:numClasses:
        ...
        disp[['Class ' num2str(options.classes{i}] ' has ' num2str[numIdxClasses(i]) ' datasets.T]);
    
    disp['Reducing to min. dataset size.T];
...
elif isfield[options, 'ageMatch'] # Perform ageMatching in decades :
    # (works only for 2 classes, and classes are equal in size afterwards)
    if options.ageMatch andand numel[CrossValidation.classesMapped == 2]:
        ageVec = zeros[size(classes]);
        for i in 1:numClasses:
            for k in 1:numel[options.classes{i}]:
                ageVec[idxClasses{i}(k]) = str2double[additional{idxClasses{i}(k], options.ageColumn});
            
        
        ageVec = floor[ageVec / 10];
        
        firstClassRand = idxClasses{1}(randperm[numel(idxClasses{1}]));
        secondClassChoice = idxClasses{2}(randperm[numel(idxClasses{2}]));
        firstClassSelect = [];
        secondClassSelect = [];
        for i in 1:numel[firstClassRand]:
            ageCur = ageVec[firstClassRand(i]);
            for k in 1:numel[secondClassChoice]:
                if ageCur == ageVec[secondClassChoice(k]):
                    firstClassSelect = [firstClassSelect; firstClassRand[i]];
                    secondClassSelect = [secondClassSelect; secondClassChoice[k]];
                    secondClassChoice = [secondClassChoice[1:k-1]; secondClassChoice[k+1:]];
                    break;
                
            
        
        
        idxClasses{1} = sort[firstClassSelect];
        idxClasses{2} = sort[secondClassSelect];
...
for i in 1:numel[idxClassesComplete]:
...
if options.patientSensitive:
    ...
    patientCount = 0;
    patientIDbefore = -1;
    for i in 1:numel[patientIDClassesCompleteSorted]:
    ...
    for i in 1:numel[patientIDClassesComplete]:
