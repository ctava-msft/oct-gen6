def createONHCircle[DataDescriptor, onhCenter, radius]:
# CREATEONHCIRCLE Creates an index image same size as the SLO image were
# area covered by the ONH (given by position and radius) is set to 1, 0
# otherwise.
# 
...
for i in 1:size[slo,1]:
    idx = find[slo(i,:] == 1);
    if numel[idx] != 0        :
        octPosStart = convertPosition[[i, idx(1]], 'SloToOctVol', DataDescriptor);
        octPosEnd = convertPosition[[i, idx(]], 'SloToOctVol', DataDescriptor);
