def mergeLayers[layers, data, OptParams, mode, dist, gradients]:
# MERGELAYERS Part of a failed approach for a new volume segmentation:
...
if strcmp[mode, 'discOptGroup']:
    ...
    disp[['There are ' num2str(numel(groups]) ' groups with up to ' num2str[size(groups{}, 1]) ' entries.T]);
...
elif strcmp[mode, 'discOptGroupGradient']:
    evalAdder = [2 2];
    genValues = OptParams.genValues;
    OptParams.evalAdder = [2 2];
