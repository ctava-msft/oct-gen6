def segmentInnerLayersCirc[bscan, Params, rpe, infl, medline, bv]:
...
for k in 1:size[alignedBScan,1]:
...
inflMaxMiddle = max[flatINFL((ceil(0.4 * ]:floor[0.6 * ])));
inflMaxLeft = mean[flatINFL(1:floor(0.05 * ]));
inflMaxRight = mean[flatINFL((ceil(0.95 * ]:)));
...
# values.
def treshDistRPE[rpe, line, tresh, range, Params]:
    dist = abs[rpe - line];
    distSorted = sort[dist(line != 0], 'ascend');
    avgDist = mean[distSorted(ceil( * Params.INNERCIRC_SEGMENT_RPESORTPART(1]):...
                                floor[ * Params.INNERCIRC_SEGMENT_RPESORTPART(2])));
    res = line;
    distWoMean = abs[dist-avgDist];
    idx = distWoMean > tresh;
    idx = bwmorph[idx,'dilate',Params.INNERCIRC_SEGMENT_RPEDILATE];
    if range[1] != 0:
        idx[1:ceil(range(1] * )) = 0;
        idx[ceil(range(2] * :)) = 0;
