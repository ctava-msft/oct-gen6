...
for j in 2:size[pmx,2]:
    for i in 1:size[pmx,1]:
        success = 0;
        ib = 1;
        while not success andand ib <= size[lines,1]:
            if (pmx[i,j] <= lines[ib,j-1] + tresh) andand  (pmx[i,j] >= lines[ib,j-1] - tresh):
                lines[ib,j] = pmx[i,j];
                success = 1;
