def extremafinder[octimg, count, mode, lineregion]:
# EXTREMAFINDER finds the positions with highest/lowest value within
# A-Scans by looking for gradient changes.
#
# EXTREMA = extremafinder[OCTIMG, COUNT, MODE, LINEREGION]
# finds the local positions with highest/lowest value within
# A-Ascans, i.e. extrema. Extrema means in this case: Change of the gradient 
...
# Compute if the gradient of the input rises or falls. Very simple :
# gradient measure, just the difference in the rows.
gradoctimg = octimg[2:, :] - octimg[1:-1, :];
gradoctimg = [zeros[1, size(octimg,2], 'single') - 1; gradoctimg];
gradoctimg[gradoctimg < 0] = -1;
gradoctimg[gradoctimg > 0] = 1;
extremaoctimg = gradoctimg[2:, :] - gradoctimg[1:-1, :];

# Delete rises/drops, if desired:
if numel[strfind(mode, 'min']) != 0 :
    extremaoctimg[extremaoctimg < 0] = 0;
elif numel[strfind(mode, 'max']) != 0 :
    extremaoctimg[extremaoctimg > 0] = 0;
...
for i in 1:size[octimg,2]:
    ...
    extremaoctimg[lineregion(2,i]+1:,i) = 0;
