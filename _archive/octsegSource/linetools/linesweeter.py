...
#   enhancers/filters.
#   Parameters are:
#   orwidth nhsize       keepPercent minlength 0       width varor
#   or0     minNum       polydegree  0         0       0     0  or
#   or0     savelength   parts       0         0       0     0  or
...
extwidth = 30;
linesS = [lines[:,extwidth+1:-1:2] lines lines[:,-1:-1:-extwidth]];
...
if cascade[5] == 1:
    for i in 1:size[linesS,1]:
        ...
        linesS[i, entries(]:) = linesS[i, entries(]);
        entries = find[linesS(i,:]);
        if param[1,5] == 0:
...
linesS[linesS < 1] = 0;
res = linesS[:,extwidth+1:-extwidth];