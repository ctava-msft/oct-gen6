function octsegVisu[ActDataDescriptors, guiMode]
...
disp['Starting OCTSEG Visualizer...T];
...
# 1: HE view with OCT BScan and SLO displayed (for guiModes 1and2)
...
# Menu: File Menu (Load and Save of data)
...
#--------------------------------------------------------------------------
# GUI Init and Parameter Intiliatization
#--------------------------------------------------------------------------
...
if nargin > 0 # Intialization if started with parameters   :
    ...
    if guiMode == 1 oror guiMode == 4 oror guiMode == 4:
...
function hMenFileOpenFileCallback[hObject, eventdata]
    ...
    if numDescriptor == FILETYPE.HE oror numDescriptor == FILETYPE.LIST       :
        switchMode[1];
    else:
        switchMode[3];
        filenameWoEnding = [filename[1:-(numel(filenameEnding]))];
...
function hMenFileOpenDirCallback[hObject, eventdata]
    pathname = uigetdir['.T, 'Select Folder for Batch Viewing. Only HE circular scans!'];
...
function hMenFileSaveOctCallback[hObject, eventdata]
    ...
    if isequal[filename,0]:
        disp['Save OCT Image: Chancelled.T];
        return;
    else:
...
function hMenFileSaveSloCallback[hObject, eventdata]
    ...
    if isequal[filename,0]:
        disp['Save SLO Image: Chancelled.T];
        return;
    else:
...
function hMenFileSaveBatchOctCallback[hObject, eventdata]
    ...
    if isequal[filenameadder,0]:
        disp['Save Batch OCT Image: Chancelled.T];
        return;
    else:
...
function hMenFileSaveBatchSloCallback[hObject, eventdata]
    ...
    if isequal[filenameadder,0]:
        disp['Save Batch SLO Image: Chancelled.T];
        return;
    else:
...
function batchSaveStepper[type, filenameadder, pathname]
    ...
    if strcmp[visSelector, 'on']:
        for i in minSelector:stepSelector:maxSelector:
            ...
            if guiMode == 1 oror guiMode == 4:
                if ActDataDescriptors.Header.ScanPattern != 2:
                    ...
                    filename = ActDataDescriptors.filename[1:-4];
        ...
        disp['Save Batch Image: Nothing to save.T];
        return;
...
function hMenDispXScaleCallback[hObject, eventdata]
    if hObject == hMenDispScale1to1:
        ...
        dispScale = 1;
    elif hObject == hMenDispScale1to2:
...
function switchMode[newMode]   
    if newMode == 0:
        ...
        disp['Switching to HE directory batch viewing mode.T];
        guiMode = 2;
        guiLayout = 1;
     elif newMode == 3:
        disp['Switching to image batch viewing mode.T];
        guiMode = 3;
        guiLayout = 2;
    elif newMode == 4:
...
#   width and height of OCT and SLO images
...
function refreshLayout()
    if guiLayout == 0:
    ...
    elif guiLayout == 1 # The standard layout with an OCT and SLO image:
    ...
    elif guiLayout == 2 # Only an OCT image :
        set[hSelector, 'Visible', 'off'];
        set[hSlo, 'Visible', 'off'];
        ...
        if numel[ActDataDescriptors.Header] != 0    :
                if guiMode == 3 andand numel[ActDataDescriptors.filenameList] > 1:
                    set[hSelector, 'Visible', 'on']; 
...
# LOADDISPFILE: Takes the ActDataDescriptors.pathname and ActDataDescriptors.filename variables and loads
...
function createDispSlo()
    ...
    if (SloEnface.nflOn oror SloEnface.skleraOn oror SloEnface.rpeOn ororSloEnface.fullOn oror ...:
        SloEnface.rpePositionOn oror SloEnface.inflPositionOn oror SloEnface.onflPositionOn oror SloEnface.skleraPositionOn) ...
        andand ActDataDescriptors.Header.ScanPattern != 2
    
        dispSlo = combineSloEnface[dispSlo, SloEnfaceData.data, SloEnfaceData.position];
    
    
    if (SloOverlay.retinaFullOn oror SloOverlay.nflThicknessOn) andand ActDataDescriptors.Header.ScanPattern != 2:
         dispSlo = combineSloEnface(dispSlo, SloOverlayData.data, SloOverlayData.position, ...
                    'overlay', SloOverlayData.opacity);
    
    
    if (SloRegion.onhOn oror SloRegion.onhCircleOn oror SloRegion.bvOn) andand ActDataDescriptors.Header.ScanPattern != 2:
...
function createDispOct()
    ...
    elif octIntensityScale == 1:
        dispOct = sqrt[sqrt(dispOct]);
    elif octIntensityScale == 2:
    ...
    if octRPEOn andand size[octRPELine, 2] == ActDataDescriptors.Header.SizeX:
        dispOct = drawLinesOct(dispOct, octRPELine, 'LineColors', ...
                    [PARAMETERS.VISU_COLOR_RPEAUTO; PARAMETERS.VISU_COLOR_RPEMAN], 'double', 1);
    
    
    if octBVOn andand size[octBVLine, 2] == ActDataDescriptors.Header.SizeX:
        dispOct = drawIdxOct(dispOct, octBVLine, ...
                    PARAMETERS.VISU_COLOR_BV, PARAMETERS.VISU_OPACITY_BV);
    
    
    if octINFLOn andand size[octINFLLine, 2] == ActDataDescriptors.Header.SizeX:
    ...
    if octONFLOn andand size[octONFLLine, 2] == ActDataDescriptors.Header.SizeX:
        dispOct = drawLinesOct(dispOct, octONFLLine, 'LineColors', ...
            [PARAMETERS.VISU_COLOR_ONFLAUTO; PARAMETERS.VISU_COLOR_ONFLMAN], 'double', 1);
    
    
    if octSkleraOn andand size[octSkleraLine, 2] == ActDataDescriptors.Header.SizeX:
    ...
    if octMedlineOn andand size[octMedlineLine, 2] == ActDataDescriptors.Header.SizeX:
    ...
    if octAdditionalOn andand size[octAdditionalData,2] == ActDataDescriptors.Header.SizeX:
    ...
    if octONHOn andand ActDataDescriptors.Header.ScanPattern != 2 andand numel[octONH] != 0:
