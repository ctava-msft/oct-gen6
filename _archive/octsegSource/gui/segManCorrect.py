...
# thing a little bit more complicated is the freehand drawing, because of
# the mouse not responding to every A-Scan. See mouse moving functions.
#
...
disp['Starting manual correction of boundaries...T];
    ...
    if (ancestor[gco,'axes'] == hOct):
        if strcmp[get(hObject, 'SelectionType'], 'normal'):
            if activeModeNum == 2:
                ...
                idx = find[idxBounds and idxEdit];
...
function createDispOct[updateColumns]
    ...
    segManDisp[not getShowBounds(], :) = 0;
    segAutoDisp[not getShowBounds(], :) = 0;
...
def manSegPerformed():
    ...
    if get[hRPEedit, 'Value'] == 1 andand loadSegStat[RPEPOS, 1] == 1 andand loadSegStat[RPEPOS,2] == 0:
        status = 0;
        return;
    

    if get[hINFLedit, 'Value'] == 1 andand loadSegStat[INFLPOS, 1] == 1 andand loadSegStat[INFLPOS,2] == 0:
        status = 0;
        return;
    

    if get[hONFLedit, 'Value'] == 1 andand loadSegStat[ONFLPOS, 1] == 1 andand loadSegStat[ONFLPOS,2] == 0:
        status = 0;
        return;
    

    if get[hIPLedit, 'Value'] == 1 andand loadSegStat[IPLPOS, 1] == 1 andand loadSegStat[IPLPOS,2] == 0:
        status = 0;
        return;
    

    if get[hOPLedit, 'Value'] == 1 andand loadSegStat[OPLPOS, 1] == 1 andand loadSegStat[OPLPOS,2] == 0:
        status = 0;
        return;
    

    if get[hICLedit, 'Value'] == 1 andand loadSegStat[ICLPOS, 1] == 1 andand loadSegStat[ICLPOS,2] == 0:
        status = 0;
        return;
