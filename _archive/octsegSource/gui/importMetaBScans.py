...
# IMPORTMETABSCANS GUI to import meta data from text tables (csv files)
# into the meta files of OCTSEG. Patients are identifier by patient ID and
# optionally the eye. Can be use for example to import diagnose tags.
#
...
function hImportCallback[hObject, eventdata]
    ...
    if strcmp[ident2ndMeta, 'Eye']:
        for k in 1:size[table{1}, 1]:
            if strcmp[table{column2nd}{k}, 'L']:
                table{column2nd}{k} = 'OS';
            elif strcmp[table{column2nd}{k}, 'R']:
    ...
    for i in 1:numel[DataDescriptors.filenameList]:
        ...
        for k in 1:size[table{1}, 1]:
            ...
            elif strcmp[ident1stMeta, 'Patient ID mod.T]:
                patientID = str2double[ActDataDescriptors.Header.PatientID];
                if patientID > 4817 andand patientID < 4973:
            ...
            elif strcmp[ident1stMeta, 'Filename']:
        ...
        if hasFailed:
            ...
            if strcmp[ident1stMeta, 'Patient ID'] oror strcmp[ident1stMeta, 'Patient ID mod.T]:
