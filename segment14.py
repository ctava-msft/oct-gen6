...
def process_and_draw_layers(bscan,layers_config):
    output_image = bscan.copy()
    output_image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
    line_coordinates = {}
    contour_info = {}

    # Load extrema results from file
    with open(f"./{output_dir}/extrema_results.json", 'r') as f:
        extrema_results = json.load(f)

    # Create a mask based on extrema results to enhance high frequencies (edges)
    rows, cols = bscan.shape
    mask = np.zeros((rows, cols), dtype=np.float32)
    for col in range(cols):
        for val in extrema_results['max'][col]:
            row = int(val * rows)
    ...
    enhanced_image = bscan * mask

    # Apply Gaussian smoothing
    smoothed_image = gaussian_filter(enhanced_image, sigma=0.9).astype(np.uint8)
...
def segment_layers(bscan, params, medline=None):
    ...
    if np.max(bscan) == np.min(bscan):
        ...
        # Integrate extrema_finder and print the results
        # extrema_results = extrema_finderNEW(bscan_normalized, num_extrema=9, mode='max')
        # with open(f"./{output_dir}/extrema_results.json", 'w') as f:
        #     f.write(f'{extrema_results}')
...
if __name__ == "__main__":
    ...
    processed_bscan = process_and_draw_layers(layers, layers_config)
