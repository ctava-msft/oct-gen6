def evaluateClassification[Results, options]:
...
for numRun in 1:Results.numFolds:
...
for i in 1:size[Results.confusionMatrix, 1]:
...
# Calculate ROC curve
if options.roc ...:
        andand numel[Results.classes] == 2 ...
        andand (strcmp[Results.ClassifyOptions.classifier, 'SVM'] ...
        oror strcmp[Results.ClassifyOptions.classifier, 'Bayes'])
    discriminators = [];
    goldStandard = [];
    ...
    for i in 1:numel[thresholdValues]:
        classes[discriminators < thresholdValues(i]) = Results.classes[2];
        classes[discriminators >= thresholdValues(i]) = Results.classes[1];
        truePositives = sum[(classes == Results.classes(2]) and (goldStandard == Results.classes[2]));
        trueNegatives = sum[(classes == Results.classes(1]) and (goldStandard == Results.classes[1]));
        falsePositives = sum[(classes == Results.classes(2]) and (goldStandard == Results.classes[1]));
        falseNegatives = sum[(classes == Results.classes(1]) and (goldStandard == Results.classes[2]));
    ...
    Results.auRoc = 0;
    for i in 2:numel[thresholdValues]:
...
for numRun in 1:Results.numFolds:
    ...
    if numel[Results.classes] == 2 ...:
        andand (strcmp[Results.ClassifyOptions.classifier, 'SVM'] oror strcmp[Results.ClassifyOptions.classifier, 'Bayes'])
