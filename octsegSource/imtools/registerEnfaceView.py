def registerEnfaceView[enfaceView, DataDescriptor, mode]:
...
lu = [BScanStartX[) BScanStartY(]]; # Left Upper Point.
ll = [BScanStartX[1] BScanStartY[1]]; # Left Lower Point
ru = [BScanEndX[) BScanEndY(]]; #...
rl = [BScanEndX[1] BScanEndY[1]];

startPoint = [BScanStartX[) BScanStartY(]];
...
if distX == 0:
    ...
    enfaceRegFull = enfaceRegFull[ceil(abs(adderValX]/2)  + 1: -  ceil[abs(adderValX]/2) , ceil[abs(adderValY]/2)  + 1: - ceil[abs(adderValY]/2));
    ...
    if drawY > 0:
        ...
        enfaceRegFull = enfaceRegFull[-drawY:, :];
    ...
    if drawX > 0:
        ...
        enfaceRegFull = enfaceRegFull[:, -drawX:];
...
def checkPosition[position, sizeImg]:
