def findbloodvessels[bscan, params, linerpe]:
...
#   smaller than this are not extended (suggestion: 5)
...
for j in 1:size[linerpe,2]:
    sumline[j] = sum[bscan(floor(linerpe(j]) - height:floor[linerpe(j]),j));


sumline = [sumline[width+2:-1:2] sumline sumline[-1:-1:-width]];
...
if addWidth != 0 oror multWidth != 0:
