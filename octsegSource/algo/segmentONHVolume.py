def segmentONHVolume[volume, params, rpe]:
...
for k in 1:size[volume, 3]:
    ...
    bScanMax = bscanSorted[floor( * params.ONH_SEGMENT_BSCANTHRESHOLD]);
...
border = 0.1;
enface[:, 1:round(( * border])) = 0;
enface[:, -round(( * border]):) = 0;
...
maxiter = 200;
iter = 0;
while sum[sum(abs(onh - onhOld])) > 0 andand iter <= maxiter:
    onhOld = onh;
    onh = bwmorph[onh, 'dilate']; 
    onh = onh and enfaceTH;
    iter = iter + 1;
...
if sum[sum(single(onh])) == 0:
    ...
    if onhCenter[1] <= 1:
        success = 0;
    elif onhCenter[1] >= size[onh, 1];:
        success = 0;
    
    
    if onhCenter[2] <= 1:
        success = 0;
    elif onhCenter[2] >= size[onh, 2];:
        success = 0;
...
if success:
    ...
    disp['ONH Segmentation failed.T];
