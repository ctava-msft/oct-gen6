def segmentONFLCirc[bscan, Params, rpe, icl, ipl, infl, bv]:
...
[alignedBScanDSqrt, not , transformLine] = alignAScans[bscanDSqrt, Params, [icl; round(infl]]);
...
for k in 1:size[alignedBScanDSqrt,1]:
...
if mod[size(alignedBScanDSqrt,1], 2) == 1 :
    alignedBScanDSqrt = alignedBScanDSqrt[1:-1, :];
...
if mod[size(alignedBScanDSqrt,2], 2) == 1:
    ...
    alignedBScanDSqrt = alignedBScanDSqrt[:, 2:];
...
# Heavily blurring the first estimate by median and gaussian filtering.
...
def estimateNoise[octimg, Params]:
