def denoisePM[img, params, edgestopfct, startsol, precision]:
...
if nargin < 2:
    error['Not enough input arguments.T];
...
while (sum[resarr] > 0) andand (iter < params.DENOISEPM_MAXITER);:
    ...
    if params.DENOISEPM_SMOOTH != 0:
        ...
        elif strcmp[edgestopfct, 'perona']:
            coeff = peronaEdgeStop[smoothsol, sigma];
        elif strcmp[edgestopfct, 'tukeylog']:
            coeff = tukeyEdgeStopLog[smoothsol, sigma];
        elif strcmp[edgestopfct, 'complex']:
    ...
    # stencil computation
    stencilN[2:-1, 2:-1] = (coeff[2:-1, 2:-1] + coeff[1:-2, 2:-1])/2;
    stencilS[2:-1, 2:-1] = (coeff[2:-1, 2:-1] + coeff[3:, 2:-1])/2;
    stencilE[2:-1, 2:-1] = (coeff[2:-1, 2:-1] + coeff[2:-1, 3:])/2;
    stencilW[2:-1, 2:-1] = (coeff[2:-1, 2:-1] + coeff[2:-1, 1:-2])/2;

    stencilM = stencilN + stencilS + stencilE + stencilW + 1;

    # solution computation: R/B Gauss Seidel
    sol[2:2:-1, 2:2:-1] = (img[2:2:-1, 2:2:-1] ...
        + (stencilN[2:2:-1, 2:2:-1] .* sol[1:2:-2, 2:2:-1] ...
        + stencilS[2:2:-1, 2:2:-1] .* sol[3:2:, 2:2:-1] ...
        + stencilE[2:2:-1, 2:2:-1] .* sol[2:2:-1, 3:2:]...
        + stencilW[2:2:-1, 2:2:-1] .* sol[2:2:-1, 1:2:-2])) ...
        ./ stencilM[2:2:-1, 2:2:-1];

    sol[3:2:, 3:2:] = (img[3:2:, 3:2:] ...
        + (stencilN[3:2:, 3:2:] .* sol[2:2:-1, 3:2:] ...
        + stencilS[3:2:, 3:2:] .* sol[4:2:, 3:2:] ...
        + stencilE[3:2:, 3:2:] .* sol[3:2:, 4:2:] ...
        + stencilW[3:2:, 3:2:] .* sol[3:2:, 2:2:-1])) ...
        ./ stencilM[3:2:, 3:2:];

    sol[2:2:-1, 3:2:] = (img[2:2:-1, 3:2:] ...
        + (stencilN[2:2:-1, 3:2:] .* sol[1:2:-2, 3:2:] ...
        + stencilS[2:2:-1, 3:2:] .* sol[3:2:, 3:2:] ...
        + stencilE[2:2:-1, 3:2:] .* sol[2:2:-1, 4:2:] ...
        + stencilW[2:2:-1, 3:2:] .* sol[2:2:-1, 2:2:-1])) ...
        ./ stencilM[2:2:-1, 3:2:];

    sol[3:2:, 2:2:-1] = (img[3:2:, 2:2:-1] ...
        + (stencilN[3:2:, 2:2:-1] .* sol[2:2:-1, 2:2:-1] ...
        + stencilS[3:2:, 2:2:-1] .* sol[4:2:, 2:2:-1] ...
        + stencilE[3:2:, 2:2:-1] .* sol[3:2:, 3:2:] ...
        + stencilW[3:2:, 2:2:-1] .* sol[3:2:, 1:2:-2])) ...
        ./ stencilM[3:2:, 2:2:-1];

    # residual computation
    resimg[2:-1, 2:-1] = (-(stencilN[2:-1, 2:-1] .* sol[1:-2, 2:-1] ...
        + stencilS[2:-1, 2:-1] .* sol[3: , 2:-1] ...
        + stencilE[2:-1, 2:-1] .* sol[2:-1, 3:] ...
        + stencilW[2:-1, 2:-1] .* sol[2:-1, 1:-2])) ...
        + stencilM[2:-1, 2:-1] .* sol[2:-1, 2:-1] - img[2:-1, 2:-1];
    ...
    sol = [sol[:,2], sol[:, 2:-1], sol[:,-1]];
    sol = vertcat[sol(2,:], sol[2:-1, :], sol[-1,:]);
...
def tukeyEdgeStopLog[img, sigma]:
