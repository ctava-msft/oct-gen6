def segmentINFLVolume[volume, params, not , rpe, medline]:
...
for i in 1:size[volume, 3]:
    [inflLin, inflLinChoice]= segmentINFLLin[volume(:,:,i], params, rpe[i,:], medline[i,:]);
    inflAuto[i,:] = inflLin;
 
    for n in 1:size[inflLinChoice, 1]:
        inflChoice[i,:, n] = inflLinChoice[n, :];
    
    
    disp[['INFL of BScan ' num2str(i] ' segmented automatically in 2D.T]);
