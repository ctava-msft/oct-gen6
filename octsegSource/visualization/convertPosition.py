def convertPosition[pos, mode, DataDescriptor]:
# CONVERTPOSITION converts positions in pixels between OCT and SLO images
# and performes boundary checks. Used for visualization in OCTSEG.
# 
# [OCTPOS SLOPOS] = convertPosition[POS, MODE, HEADER, BSCANHEADER]
# OCTPOS, SLOPOS: The new positions, in pixel values
...
#                 to HE and OCTSEG specifications. Entries:
#                 - Header
...
if strcmp[mode, 'SloToOctVol']:
    ...
    if distX == 0:
        ...
        octPos[2] = octSize[2] - (rwPos[1] - min[[startY(1] startY[)]]) / DataDescriptor.Header.Distance;
...
def toRealWorldSlo[xSlo,ySlo, sloScale]:
    xReal = xSlo * sloScale[1];
    yReal = ySlo * sloScale[2];


def toSloFromRealWorld[xReal,yReal, sloScale]:
    xSlo = xReal / sloScale[1];
    ySlo = yReal / sloScale[2];


def toRealWorldOct[aScanNumber, octSize, xStart, yStart, xEnd, yEnd]:
...
def toOctFromRealWorld[xReal, yReal, octSize, startX, startY, endX, endY]: 
    lu = [startX[) startY(]]; # left upper point
    ru = [endX[) endY(]]; # ...
