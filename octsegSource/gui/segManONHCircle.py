...
# SEGMANONHCIRCLE Manual correction of the automated ONH segmentations (only
# center and radius.
#
# Due to the lack of data and time the volume segmentation approach was not 
# pushed further. Now (2015) I would suggest to use other methods.
#
...
hSave = uicontrol('Style','pushbutton','String','Save and Quit',...
    'FontSize', 10,...
    'Units','pixels',...
...
function hRadiusEditCallback[hObject, eventdata]
    ...
    if onhRadiusTemp > ONHRADIUSMIN andand onhRadiusTemp < ONHRADIUSMAX:
...
function loadSeg()
    ...
    if status == 2:
        ...
        disp['No ONH radius segmentation performed yet.T];
        return;
    ...
    if status == 2:
        ...
        disp['No ONH center segmentation performed yet.T];
        return;
...
def reloadMetaDataTagEnface[tags]:
