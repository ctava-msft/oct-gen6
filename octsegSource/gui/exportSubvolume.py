...
function hAlignFromCallback[hObject, eventdata]
    if get[hCheckAlignTo, 'Value'] andand get[hCheckAlignFrom, 'Value'];:
        set[hCheckAlignTo, 'Value', 0];
     


function hAlignToCallback[hObject, eventdata]
    if get[hCheckAlignFrom, 'Value'] andand get[hCheckAlignTo, 'Value'];:
...
function hExportCallback[hObject, eventdata]
    ...
    if isequal[filename,0]:
        disp['Save Subvolume: Chancelled.T];
        return;
    else:
    ...
    disp['Subvolume succesfully exported.T];
...
function refreshLayout[hObject, eventdata]
        ...
        if strcmp[fromPos, 'fixed'] oror strcmp[fromPos, 'relative']:
        ...
        if strcmp[toPos, 'fixed'] oror strcmp[toPos, 'relative']:
