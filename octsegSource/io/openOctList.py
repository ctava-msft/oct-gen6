...
if numel[strfind(options, 'header']) == 0:
    ...
    if size[dataFirst,1] < cutVal oror numel[strfind(options, 'nocut']) != 0:
    ...
    for i in 2:maxfcount:
        dataAkt = single[imread([pathOnly flist{i}]]);
        if size[dataAkt, 3] == 3:
            dataAkt = mean[single(dataAkt], 3);
        
        if size[dataAkt,1] < cutVal oror  numel[strfind(options, 'nocut']) != 0:
            BScans[:,:,i] = dataAkt;
        else:
