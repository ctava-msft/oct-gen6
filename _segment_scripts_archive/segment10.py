...
def fill_scaled_contour(layer_image, scaled_contour):
...
def process_and_draw_layers(bscan):
    ...
    def process_layers(bscan, contours, layer_colors, scaling_factors):
        ...
        for layer_idx, multiplier in scaling_factors.items():
            ...
            try:
                scaled_contour, original_area, scaled_area = scale_contour(contour, multiplier)
            ...
            contour_info[layer_idx] = {
                "original_contour": contour.tolist(),
                "scaled_contour": scaled_contour.tolist(),
            ...
            cv2.drawContours(output_image, [scaled_contour], -1, color, 2)
            
            filled_data = fill_scaled_contour(bscan, scaled_contour)
            ...
            line_coordinates[layer_idx] = scaled_contour.tolist()
    ...
    filtered_image_with_lines = process_layers(bscan, contours, layer_colors, scaling_factors)
...
if __name__ == "__main__":
    ...
    rnfl_enlarge = segment_layers(image, params)
    print("./tests/rnfl_enlarge.png")
    cv2.imwrite(f"./tests/rnfl_enlarge.png", rnfl_enlarge)